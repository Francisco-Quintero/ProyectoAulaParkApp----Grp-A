...
namespace ParkApp
{
    public partial class TipoVehiculo1 : Form
    {
        ...
        public void Mostrar()
        {
            // Llamar al servicio para obtener la lista de TipoVehiculo
            List<TipoVehiculo> listaTipoVehiculos = servicioTipoVehiculo.Listar();

            // Verificar si la lista contiene elementos
            if (listaTipoVehiculos.Count > 0)
            {
                // Limpiar el DataGridView antes de añadir nuevos elementos
                dataGridGestionVehiculos.DataSource = null;
                dataGridGestionVehiculos.DataSource = listaTipoVehiculos;

                MessageBox.Show("Consulta exitosa.");
            }
            else
            {
                MessageBox.Show("No se encontraron tipos de vehículos.");
            }
